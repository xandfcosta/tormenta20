...
// O contador tem CINCO estados, e a ORDEM entre eles é regra: a cena existe
...
func TestTheCounterHasFiveStates(t *testing.T) {
	acao := &Scene{Kind: SceneAction, Number: 1}
	casos := []struct {
		nome   string
		cena   *Scene
		rodada int
		...
		{"em combate", acao, 2, 1, 4, "Rodada 2 · Turno 2/4"},
		{"o turno é 1-indexado na tela", acao, 1, 0, 3, "Rodada 1 · Turno 1/3"},
	}
	for _, c := range casos {
