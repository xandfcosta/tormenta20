...
func TestTheInitiativeBonusIsTheExpertiseTotal(t *testing.T) {
	sheet := ComputedSheet{Expertises: []ExpertiseBreakdown{
		{Name: "Atletismo", Total: 12},
		{Name: "Iniciativa", Total: 8},
		{Name: "Percepção", Total: 5},
	}}
	if bonus := InitiativeTotal(sheet); bonus != 8 {
...
func TestASheetWithoutTheExpertiseRollsTheBareD20(t *testing.T) {
	...
	noInitiative := ComputedSheet{Expertises: []ExpertiseBreakdown{{Name: "Atletismo", Total: 12}}}
	if bonus := InitiativeTotal(noInitiative); bonus != 0 {
...
func TestTheLookupIsExactOnTheCatalogSpelling(t *testing.T) {
	sheet := ComputedSheet{Expertises: []ExpertiseBreakdown{{Name: "iniciativa", Total: 8}}}
	if bonus := InitiativeTotal(sheet); bonus != 0 {
