...
func TestSpellPmLimit(t *testing.T) {
	...
	arcanist12 := char(12, CharacterClass{ClassName: "Arcanista", Level: 12})
	...
	t.Run("nível NA CLASSE que fornece a magia, cheio", func(t *testing.T) {
		...
		if got := SpellPmLimit(arcanist12, 0, []string{"Arcanista"}); got != 12 {
	...
	t.Run("multiclasse conta só a classe que fornece", func(t *testing.T) {
		bardArcanist := char(10, CharacterClass{ClassName: "Guerreiro", Level: 9}, CharacterClass{ClassName: "Arcanista", Level: 1})

		if got := SpellPmLimit(bardArcanist, 0, []string{"Arcanista"}); got != 1 {
	...
	t.Run("fonte que não é classe usa o nível de personagem", func(t *testing.T) {
		barbarian8 := char(8, CharacterClass{ClassName: "Bárbaro", Level: 8})

		if got := SpellPmLimit(barbarian8, 0, []string{"Arcanista"}); got != 8 {
			t.Errorf("limite = %d, want 8", got)
		}
	})

	t.Run("bônus de item soma ao teto", func(t *testing.T) {
		if got := SpellPmLimit(arcanist12, 2, []string{"Arcanista"}); got != 14 {
