...
func NewGate(banco *sql.DB, q *sqlcgen.Queries, cfg config.Config) Gate {
	return Gate{db: banco, queries: q, cfg: cfg}
}
...
// um oráculo de tempo.
func (g Gate) Authenticate(ctx context.Context, email, senha string) (sqlcgen.User, error) {
	user, err := g.queries.GetUserByEmail(ctx, wire.NormalizeEmail(email))
	if err != nil {
		return sqlcgen.User{}, ErrBadCredentials
	}
	if bcrypt.CompareHashAndPassword([]byte(user.Passwordhash), []byte(senha)) != nil {
		return sqlcgen.User{}, ErrBadCredentials
	}
	return user, nil
}
...
func (g Gate) Register(ctx context.Context, corpo account.RegisterBody) (sqlcgen.User, error) {
	...
	corpo.Email = wire.NormalizeEmail(corpo.Email)
	convite, err := g.registrationInvite(ctx, corpo.Email, corpo.InviteToken)
	if err != nil {
		return sqlcgen.User{}, err
	}
	hash, err := bcrypt.GenerateFromPassword([]byte(corpo.Password), bcryptCost)
	if err != nil {
	...
	return g.createUser(ctx, sqlcgen.CreateUserParams{
		Email:        corpo.Email,
		Name:         dbvalue.NullString(corpo.Name),
...
func (g Gate) registrationInvite(
	...
	convite, ok := g.usableInvite(ctx, token)
	if !ok {
...
func (g Gate) usableInvite(ctx context.Context, token string) (sqlcgen.AccountInvite, bool) {
	...
	convite, err := g.queries.GetAccountInvite(ctx, token)
	if err != nil || convite.Usedat.Valid {
		return sqlcgen.AccountInvite{}, false
	}
	vence, err := time.Parse(dbvalue.IsoLayout, convite.Expiresat)
...
// atrás junto.
//
// `convite` é nulo quando um endereço de `ADMIN_EMAILS` cria a própria conta.
func (g Gate) createUser(
	ctx context.Context, params sqlcgen.CreateUserParams, convite *sqlcgen.AccountInvite,
) (sqlcgen.User, error) {
	...
	if convite != nil {
		if err := spend(ctx, q, convite.ID, user.ID); err != nil {
			return sqlcgen.User{}, err
		}
	}
...
func spend(ctx context.Context, q *sqlcgen.Queries, conviteID, userID int64) error {
	linhas, err := q.SpendAccountInvite(ctx, sqlcgen.SpendAccountInviteParams{
		Usedat: sql.NullString{String: dbvalue.NowISO(), Valid: true},
		Usedby: sql.NullInt64{Int64: userID, Valid: true},
		ID:     conviteID,
	})
	if err != nil {
		return fmt.Errorf("gastar o convite %d: %w", conviteID, err)
	}
	if linhas == 0 {
		return ErrBadInvite
	}
...
func (g Gate) SignSession(user sqlcgen.User) (string, error) {
	...
	assinado, err := jwt.NewWithClaims(jwt.SigningMethodHS256, claims).SignedString([]byte(g.cfg.JWTSecret))
	if err != nil {
