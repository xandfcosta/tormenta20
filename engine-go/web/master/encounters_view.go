...
// (`PartyChallengeLevel`, `EncounterDifficulty`, `EncounterXP`). Esta camada resolve
// ids em verbetes e monta o que a tela lê.

// encounterRow é uma linha da composição: um verbete e quantos dele.
...
// encounterAddress monta o link que o botão de copiar entrega.
//
...
// rowsFromURL lê `?c=goblin:4,ogro:1`.
//
...
func roundedCR(nd float64) string {
...
var toneTint = map[string]string{
	"calmo":   "var(--hp-full)",
...
func difficultyColor(tom string) string {
	if c, ok := toneTint[tom]; ok {
		return "color: " + c
	}
	return "color: var(--grimorio-gold)"
}

// xpWritten com separador de milhar pt-BR: 1000 vira "1.000".
func xpWritten(n int) string {
	s := strconv.Itoa(n)
	if len(s) <= 3 {
		return s
	}
