...
const listActiveEffectsByCharacters = `-- name: ListActiveEffectsByCharacters :many
SELECT id, characterId, catalogId, scope, modifiers, createdAt
FROM active_effects WHERE characterId IN (/*SLICE:ids*/?) ORDER BY id ASC
`

type ListActiveEffectsByCharactersRow struct {
	ID          int64  `json:"id"`
	Characterid int64  `json:"characterid"`
	Catalogid   string `json:"catalogid"`
	Scope       string `json:"scope"`
...
func (q *Queries) ListActiveEffectsByCharacters(ctx context.Context, ids []int64) ([]ListActiveEffectsByCharactersRow, error) {
	query := listActiveEffectsByCharacters
	...
	defer rows.Close()
	items := []ListActiveEffectsByCharactersRow{}
	for rows.Next() {
		var i ListActiveEffectsByCharactersRow
		if err := rows.Scan(
			&i.ID,
			&i.Characterid,
			&i.Catalogid,
