...
func TestSessionsComeFromTheNewestToTheOldest(t *testing.T) {
	...
	v, err := campaigns.New(s.campaignsHost(), s.sessionAccess(), s.campaignDirectory()).LoadOne(context.Background(), dono, s.ehAdmin(t, dono), campanha, "")
	if err != nil {
...
func TestAPlayerAskingForConfigFallsBackToTheOverview(t *testing.T) {
	...
	v, err := campaigns.New(s.campaignsHost(), s.sessionAccess(), s.campaignDirectory()).LoadOne(context.Background(), visitante, s.ehAdmin(t, visitante), campanha, "config")
	if err != nil {
		t.Fatalf("carregar: %v", err)
	}
	if v.AbaAtiva() != "visao" {
...
func TestTheSwitchTogglesWhatIsInForceAndNotTheOpposite(t *testing.T) {
	...
	v, _ := campaigns.New(s.campaignsHost(), s.sessionAccess(), s.campaignDirectory()).LoadOne(context.Background(), dono, s.ehAdmin(t, dono), campanha, "config")
