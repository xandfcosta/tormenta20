...
func TestTheSceneFiltersBySearchOverNameAndSynopsis(t *testing.T) {
	...
	porNome, err := campaigns.New(f.s.campaignsHost(), f.s.sessionAccess(), f.s.campaignDirectory(), f.s.campaignLifecycle(), f.s.campaignSeating()).LoadList(context.Background(), f.dono, f.s.ehAdmin(t, f.dono), "queda", "todas")
	if err != nil {
	...
	porSinopse, err := campaigns.New(f.s.campaignsHost(), f.s.sessionAccess(), f.s.campaignDirectory(), f.s.campaignLifecycle(), f.s.campaignSeating()).LoadList(context.Background(), f.dono, f.s.ehAdmin(t, f.dono), "tormenta", "todas")
	if err != nil {
		t.Fatalf("carregar: %v", err)
	}
...
func TestTheCursorIsBornOnTheFirstOfTheFilteredList(t *testing.T) {
	...
	v, err := campaigns.New(f.s.campaignsHost(), f.s.sessionAccess(), f.s.campaignDirectory(), f.s.campaignLifecycle(), f.s.campaignSeating()).LoadList(context.Background(), f.dono, f.s.ehAdmin(t, f.dono), "wynlla", "todas")
	if err != nil {
		t.Fatalf("carregar: %v", err)
	}
...
func TestTheSceneTellsAnEmptyListFromASearchWithNoResult(t *testing.T) {
	vazia := novaCena(t)
	semNada, err := campaigns.New(vazia.s.campaignsHost(), vazia.s.sessionAccess(), vazia.s.campaignDirectory(), vazia.s.campaignLifecycle(), vazia.s.campaignSeating()).LoadList(context.Background(), vazia.eu(t).ID, vazia.eu(t).IsAdmin, "", "todas")
	if err != nil {
		t.Fatalf("carregar: %v", err)
	}
	...
	f := novaCena(t)
	f.campanha(t, "A Queda de Tauron", "")
	semResultado, err := campaigns.New(f.s.campaignsHost(), f.s.sessionAccess(), f.s.campaignDirectory(), f.s.campaignLifecycle(), f.s.campaignSeating()).LoadList(context.Background(), f.dono, f.s.ehAdmin(t, f.dono), "zzzzz", "todas")
	if err != nil {
		t.Fatalf("carregar: %v", err)
	}
...
func TestALiveSessionGoesToTheRightCampaign(t *testing.T) {
	...
	v, err := campaigns.New(f.s.campaignsHost(), f.s.sessionAccess(), f.s.campaignDirectory(), f.s.campaignLifecycle(), f.s.campaignSeating()).LoadList(context.Background(), f.dono, f.s.ehAdmin(t, f.dono), "", "todas")
	if err != nil {
		t.Fatalf("carregar: %v", err)
	}
...
func TestAnInvalidRoleInTheUrlDoesNotHideTheList(t *testing.T) {
	...
	for _, papel := range []string{"", "mestre", "GM", "'; drop table"} {
		v, err := campaigns.New(f.s.campaignsHost(), f.s.sessionAccess(), f.s.campaignDirectory(), f.s.campaignLifecycle(), f.s.campaignSeating()).LoadList(context.Background(), f.dono, f.s.ehAdmin(t, f.dono), "", papel)
		if err != nil {
			t.Fatalf("carregar: %v", err)
		}
...
func TestTheRoleFilterSeparatesRunningFromPlaying(t *testing.T) {
	...
	mestrando, err := campaigns.New(f.s.campaignsHost(), f.s.sessionAccess(), f.s.campaignDirectory(), f.s.campaignLifecycle(), f.s.campaignSeating()).LoadList(context.Background(), f.dono, f.s.ehAdmin(t, f.dono), "", "gm")
	if err != nil {
		t.Fatalf("carregar: %v", err)
	}
	...
	jogando, err := campaigns.New(f.s.campaignsHost(), f.s.sessionAccess(), f.s.campaignDirectory(), f.s.campaignLifecycle(), f.s.campaignSeating()).LoadList(context.Background(), f.dono, f.s.ehAdmin(t, f.dono), "", "player")
	if err != nil {
		t.Fatalf("carregar: %v", err)
	}
