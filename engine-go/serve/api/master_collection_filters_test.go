...
func TestTheFilterIsAnAddress(t *testing.T) {
	...
	body := pedeNoMestre(t, s, eu, "GET", "/mestre/magias?circulo=3&escola=evocacao", "").Body.String()
	if !strings.Contains(body, "6 entradas") {
...
func TestAFilterFromAnotherTabDoesNotApply(t *testing.T) {
	...
	body := pedeNoMestre(t, s, eu, "GET", "/mestre/condicoes?circulo=3", "").Body.String()
	if !strings.Contains(body, "35 entradas") {
...
func TestSearchHidesTheFilters(t *testing.T) {
	...
	noSearch := pedeNoMestre(t, s, eu, "GET", "/mestre/magias", "").Body.String()
	withSearch := pedeNoMestre(t, s, eu, "GET", "/mestre/magias?busca=fogo", "").Body.String()

	if !strings.Contains(noSearch, "Filtrar por Círculo") {
		t.Error("a cena das magias não desenha os filtros")
	}
	if strings.Contains(withSearch, "Filtrar por Círculo") {
...
func TestTheSpellCardTellsAndLetsYouOpenTheSchool(t *testing.T) {
	...
	body := pedeNoMestre(t, s, eu, "GET", "/mestre/magias?entrada=bola-de-fogo", "").Body.String()
	if !strings.Contains(body, "Evocação") {
		t.Error("o cartão da magia não diz a escola dela")
	}
	if !strings.Contains(body, "/mestre/escolas?entrada=evocacao") {
		t.Error("a escola não virou elo")
	}
	// E o destino existe, com a definição do livro.
	school := pedeNoMestre(t, s, eu, "GET", "/mestre/escolas?entrada=evocacao", "").Body.String()
	if !strings.Contains(school, "manipulam ou geram energia pura") {
		t.Error("o verbete da escola não traz a definição do livro")
	}
	// A ABREVIATURA que as tabelas do livro imprimem.
	if !strings.Contains(school, "Evoc") {
...
func TestTheClassLinksTheExpertisesItTrains(t *testing.T) {
	...
	body := pedeNoMestre(t, s, eu, "GET", "/mestre/classes?entrada=bardo", "").Body.String()
	if !strings.Contains(body, "/mestre/pericias?entrada=atuacao") {
		t.Error("o Bardo não liga a perícia que ele treina")
	}
	// E o caminho de volta: a perícia diz quem a treina de saída.
	expertise := pedeNoMestre(t, s, eu, "GET", "/mestre/pericias?entrada=atuacao", "").Body.String()
	if !strings.Contains(expertise, "/mestre/classes?entrada=bardo") {
