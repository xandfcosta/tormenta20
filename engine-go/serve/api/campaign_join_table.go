...
func (rules campaignRules) joinTable(ctx context.Context, p joinRequest) (sqlcgen.CampaignMember, error) {
	...
	// NEGA. O `return err` aqui é o que garante isso.
	if papel == "player" {
	...
	// uma CÓPIA dele. A deduplicação é por "este molde já foi copiado aqui" e não
	// por participação do molde — o molde nunca é membro.
