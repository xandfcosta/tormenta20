...
func TestTheCardAlreadyCarriesTheTableName(t *testing.T) {
	...
	v, err := campaigns.New(s.campaignsHost(), s.sessionAccess(), s.campaignDirectory(), s.campaignLifecycle(), s.campaignSeating()).LoadJoin(context.Background(), visitante, "o-token-certo")
	if err != nil {
...
func TestADeadInviteBecomesASentenceAndNotABrokenPage(t *testing.T) {
	...
	v, err := campaigns.New(s.campaignsHost(), s.sessionAccess(), s.campaignDirectory(), s.campaignLifecycle(), s.campaignSeating()).LoadJoin(context.Background(), visitante, "nao-existe")
	if err != nil {
