...
func TestGarbageInThePageFallsBackToTheFirst(t *testing.T) {
	book := bookui.BookAddress{Base: "/livro?v=abc", Opening: 6}
	for _, target := range []string{
		...
		v := readerFromRequest(httptest.NewRequest("GET", target, nil), book)
		if v.Page != 1 {
			t.Errorf("%s abriu na página %d, e o torto tem de cair na primeira", target, v.Page)
		}
	}
	// O CONTROLE: um número BOM passa. Sem ele, um `readerFromRequest` que
	// devolvesse 1 sempre passaria em todos os casos acima.
	v := readerFromRequest(httptest.NewRequest("GET", "/livro/ler?p=289", nil), book)
	if v.Page != 289 {
...
func TestAForeignRefererDoesNotBecomeABackLink(t *testing.T) {
	...
	for _, referer := range []string{
		...
		if v := readerFromRequest(req, book); v.Back != "/" {
	...
	if v := readerFromRequest(req, book); v.Back != "/mestre/bestiario" {
