...
func TestTheTotalIsTheSumOfTheLegs(t *testing.T) {
	line := polylineReading([]engine.Square{{}, {X: 3}})
	if line["ruler_text"] != rulerReading(engine.Measure(engine.Square{}, engine.Square{X: 3})) {
		t.Errorf("a régua de duas paradas deixou de ser a régua de sempre: %q", line["ruler_text"])
	}

	// Três paradas: 3 + 4 = 7 quadrados de caminho.
	path := polylineReading([]engine.Square{{}, {X: 3}, {X: 3, Y: 4}})
	if !strings.HasPrefix(path["ruler_text"].(string), "7 quadrados") {
		t.Errorf("o total de 3+4 pernas saiu %q, esperado 7 quadrados", path["ruler_text"])
	}
	if labels := path["ruler_labels"].([]string); len(labels) != 2 {
		t.Errorf("três paradas deram %d rótulos, esperado 2 pernas: %v", len(labels), labels)
	}
}
...
func TestTheLegLabelComesInMetres(t *testing.T) {
	reading := polylineReading([]engine.Square{{}, {X: 6}})
	labels := reading["ruler_labels"].([]string)
	if len(labels) != 1 || labels[0] != "9,0m" {
		t.Errorf("a perna de 6 quadrados saiu %v, esperado [\"9,0m\"] — 6 × 1,5m (p236)", labels)
	}
	// E o TOTAL continua trazendo as duas unidades e a faixa.
	total := reading["ruler_text"].(string)
	for _, chunk := range []string{"6 quadrados", "9,0m", "alcance"} {
		if !strings.Contains(total, chunk) {
			t.Errorf("a frase do total perdeu %q: %q", chunk, total)
		}
	}
}
...
func TestAZeroLengthLegHasNoLabel(t *testing.T) {
	reading := polylineReading([]engine.Square{{X: 4, Y: 4}, {X: 4, Y: 4}})
	if labels := reading["ruler_labels"].([]string); len(labels) != 1 || labels[0] != "" {
		t.Errorf("a perna de zero saiu %v, esperado um rótulo VAZIO", labels)
	}
}
