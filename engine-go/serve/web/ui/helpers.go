...
// por isso que o `buttonClasses` teve de ficar lá.
...
//
// Ele existe porque ARIA não aceita a ausência como "false": `aria-checked`
// sem valor é um estado INDEFINIDO para o leitor de tela, não um interruptor
// desligado. É a mesma armadilha que o `data-attr` booleano do Datastar tem do
// outro lado — atributo que some não diz "não".
//
// Morava na crônica e quatro famílias o liam de lá (ALE-278); todos os doze
// usos são `aria-checked` ou `aria-pressed`, e é por isso que o nome diz ARIA
// em vez de prometer uma conversão de bool para texto qualquer.
