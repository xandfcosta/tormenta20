...
# (`serve/web/assets/src/scene.ts`) e o produto é embutido no binário pelo `go:embed`.
...
./node_modules/.bin/vite build --config vite.config.ts --logLevel warn
...
cp node_modules/pdfjs-dist/build/pdf.worker.min.mjs \
   ./serve/web/assets/static/pdf.worker.js
