...
func TestStowingTakesItFromTheHandAndPutsItInTheGrid(t *testing.T) {
	...
	if recusa := bagCommand(t, f, id, fmt.Sprintf("itens/%d/guarda", item)); recusa != "" {
...
func TestEquippingRespectsTheItemAxis(t *testing.T) {
	...
	recusa := bagCommand(t, f, id, fmt.Sprintf("itens/%d/equipa/vested", item))
	...
	if recusa := bagCommand(t, f, id, fmt.Sprintf("itens/%d/equipa/wielded", item)); recusa != "" {
...
func TestEquippingRespectsTheTwoHandCeiling(t *testing.T) {
	...
	itemSemeia(t, f, id, "espada-longa", "Espada longa", "wielded")
	itemSemeia(t, f, id, "adaga", "Adaga", "wielded")
	terceiro := itemSemeia(t, f, id, "clava", "Clava", "")

	recusa := bagCommand(t, f, id, fmt.Sprintf("itens/%d/equipa/wielded", terceiro))
	if recusa == "" {
	...
	if got := equipped(t, f, terceiro); got != "" {
		t.Errorf("a recusa equipou assim mesmo: %q", got)
	}
}
...
func TestAnItemFromAnotherSheetCannotBeMoved(t *testing.T) {
	...
	if got := equipped(t, f, alheio); got != "wielded" {
...
func TestMoneyIsReceivedSpentAndCorrected(t *testing.T) {
	...
	if recusa := money(t, f, id, "gastar", 0.3); recusa != "" {
	...
	if recusa := money(t, f, id, "receber", 0.2); recusa != "" {
	...
	if recusa := money(t, f, id, "corrigir", 42); recusa != "" {
...
func TestTheGridFiltersBySearchAndByCategory(t *testing.T) {
	...
	if strings.Contains(screenSaved(comBusca), "Espada longa") {
	...
	if !strings.Contains(screenSaved(comChip), "Espada longa") {
		t.Error("o chip de armas escondeu a espada")
	}
	if strings.Contains(screenSaved(comChip), "Bálsamo restaurador") {
