...
func putPlayerTracker(st *Server, c commandCtx) (*aovivo.SessionRuntimeState, error) {
	...
	escolhido := that(combatentes, characterID)
	if escolhido == nil {
...
func that(combatentes []combatant, characterID int64) *combatant {
...
func openSheetCast(m tableMember) string {
	return fmt.Sprintf("document.getElementById('elenco-%d').showModal()", m.CharacterID)
}

func closeSheetCast(m tableMember) string {
...
func poeNaFila(v tableView, m tableMember) string {
