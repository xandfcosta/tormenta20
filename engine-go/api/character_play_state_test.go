...
func TestDescansoDeCenaLimpaSoOQueEDaCena(t *testing.T) {
	...
	if rec := authed(t, f.s, f.dono, http.MethodPost, "/characters/"+id64(f.heroi)+"/end-scene", ""); rec.Code != http.StatusOK {
		t.Fatalf("descanso de cena: code=%d — %s", rec.Code, rec.Body.String())
	}
...
func TestDescansoDeDiaLimpaOsDoisEscopos(t *testing.T) {
	...
	if rec := authed(t, f.s, f.dono, http.MethodPost, "/characters/"+id64(f.heroi)+"/end-day", ""); rec.Code != http.StatusOK {
		t.Fatalf("descanso de dia: code=%d — %s", rec.Code, rec.Body.String())
	}
