...
var asLabelsDosAtributos = map[string]string{
...
func (s *Server) handleForjaAtributos(w http.ResponseWriter, r *http.Request) {
	v, status, err := s.carregaAtributosDaForja(r, "")
	if err != nil {
		http.Error(w, err.Error(), status)
		return
	}
	s.escreveOsAtributos(w, r, v)
}
...
func (s *Server) handleForjaAtributoPasso(w http.ResponseWriter, r *http.Request) {
	recusa, status, err := s.mexeNoAtributo(r)
	if err != nil {
		http.Error(w, err.Error(), status)
		return
	}
	v, status, err := s.carregaAtributosDaForja(r, recusa)
	if err != nil {
		http.Error(w, err.Error(), status)
		return
	}
...
func (s *Server) mexeNoAtributo(r *http.Request) (recusa string, status int, err error) {
	row, status, err := s.oHeroiDaForja(r)
	if err != nil {
		return "", status, err
	}
	...
	espalhamento := oEspalhamentoDoHeroi(row)
	...
	if avisos := engine.PointBuyWarnings(espalhamento); len(avisos) > 0 {
		return aRecusaDaCompra(avisos[0]), http.StatusOK, nil
	}
	if err := s.gravaOsAtributos(r.Context(), row.ID, espalhamento); err != nil {
...
func (s *Server) gravaOsAtributos(ctx context.Context, id int64, espalhamento map[string]int) error {
	return s.queries.SetCharacterAttributes(ctx, sqlcgen.SetCharacterAttributesParams{
...
func (s *Server) oHeroiDaForja(r *http.Request) (sqlcgen.Character, int, error) {
...
func (s *Server) carregaAtributosDaForja(r *http.Request, recusa string) (attributesView, int, error) {
	row, status, err := s.oHeroiDaForja(r)
	if err != nil {
...
func aLinhaDoAtributo(
	chave string, espalhamento map[string]int, sheet engine.ComputedSheetV2, gasto int,
) attributeRow {
	return attributeRow{
		Key: chave, Label: asLabelsDosAtributos[chave],
		Base:     espalhamento[chave],
		Total:    sheet.Attributes[chave].Total,
		CanRaise: oPassoCabe(chave, espalhamento, +1),
		CanLower: oPassoCabe(chave, espalhamento, -1),
	}
}

// oPassoCabe pergunta ao motor se o espalhamento continuaria legal com o passo.
func oPassoCabe(chave string, espalhamento map[string]int, passo int) bool {
...
func (s *Server) escreveOsAtributos(w http.ResponseWriter, r *http.Request, v attributesView) {
	if r.Header.Get("datastar-request") != "" {
		fragmento, err := renderFragmento(r.Context(), attributesBody(v))
		if err != nil {
