...
func TestIniciarSignificaTresCoisas(t *testing.T) {
	...
	if _, err := f.s.IniciaASessao(ctx, sess); err != nil {
	...
	if _, err := f.s.IniciaASessao(ctx, ativa); err != nil {
	...
	if _, err := f.s.EncerraASessao(ctx, ativa); err != nil {
	...
	if _, err := f.s.IniciaASessao(ctx, encerrada); err != nil {
...
func TestEncerrarUmaSessaoQueNuncaComecouERecusado(t *testing.T) {
	...
	_, err = f.s.EncerraASessao(ctx, sess)
	...
	encerrada, _ := f.s.queries.GetSession(ctx, f.sessionID)
	if _, err := f.s.EncerraASessao(ctx, encerrada); err != nil {
...
func TestReiniciarOCombateEsvaziaAFilaESoIsso(t *testing.T) {
	...
	if err := f.s.ReiniciaOCombate(ctx, f.sessionID); err != nil {
