...
func TestAdminReachesAnotherPlayersCampaign(t *testing.T) {
	...
	t.Run("abre a campanha como gm", func(t *testing.T) {
		...
		if papel, _ := jsonField(t, rec, "role").(string); papel != "gm" {
			t.Errorf("role = %q, esperado gm (%s)", papel, rec.Body.String())
		}
...
func TestAPlayersCampaignIsNotMarkedWithTheOwner(t *testing.T) {
	...
	if papel, _ := jsonField(t, rec, "role").(string); papel != "player" {
		t.Errorf("role = %q, esperado player", papel)
	}
	if owner := jsonField(t, rec, "ownerName"); owner != nil {
