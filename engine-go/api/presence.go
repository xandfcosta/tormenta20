...
// of their sockets is a GM). Mirrors the frontend PresenceUser + the `presence` broadcast.
...
// bookkeeping (the gateway owns the socket.io broadcast); rebuilt from live sockets, so a
...
type presenceRegistry struct {
	...
	// socketID → set of sessions it joined, so a disconnect cleans every room without
	// relying on the socket lib's post-disconnect room set.
	socketSessions map[string]map[int64]bool
}
...
// join records a socket in a session and returns the session's deduped roster.
func (p *presenceRegistry) join(sessionID int64, socketID string, user PresenceUser) []PresenceUser {
	p.mu.Lock()
	defer p.mu.Unlock()
	...
	room[socketID] = user
	sessions := p.socketSessions[socketID]
	if sessions == nil {
		sessions = map[int64]bool{}
		p.socketSessions[socketID] = sessions
	}
	sessions[sessionID] = true
	return p.rosterLocked(sessionID)
}

// leave removes a socket from one session. Returns the new roster and true, or (nil,false)
// when the socket wasn't present (nothing to announce).
func (p *presenceRegistry) leave(sessionID int64, socketID string) ([]PresenceUser, bool) {
	p.mu.Lock()
	defer p.mu.Unlock()
	if sessions := p.socketSessions[socketID]; sessions != nil {
		delete(sessions, sessionID)
	}
	if !p.dropLocked(socketID, sessionID) {
		return nil, false
	}
...
func (p *presenceRegistry) disconnect(socketID string) []sessionRoster {
	p.mu.Lock()
	defer p.mu.Unlock()
	sessions := p.socketSessions[socketID]
	if sessions == nil {
	...
	delete(p.socketSessions, socketID)
	return changed
}
...
func (p *presenceRegistry) dropLocked(socketID string, sessionID int64) bool {
	room := p.presence[sessionID]
	if room == nil {
		return false
	}
	if _, ok := room[socketID]; !ok {
		return false
	}
