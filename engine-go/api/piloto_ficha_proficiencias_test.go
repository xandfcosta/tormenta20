...
func oGuerreiro(t *testing.T) (pilotoFixture, int64) {
	t.Helper()
	f := novoPiloto(t)
...
func asGuardadas(t *testing.T, f pilotoFixture, id int64) map[string]bool {
	t.Helper()
	row, err := f.s.queries.GetCharacter(context.Background(), id)
	if err != nil {
		t.Fatalf("ler o personagem: %v", err)
	}
	return asProficienciasGuardadas(row.Proficiencies)
}

// gravaNaMao põe um estado que só um ajuste manual produziria.
func gravaNaMao(t *testing.T, f pilotoFixture, id int64, blob string) {
	t.Helper()
...
//
// A trava mora no `comandoDaFicha`, e a fatia 1 já a prende uma vez. O que ESTE
...
// comando escrito sem o `?tab=` faz o `aAbaPedida` cair na primeira aba: o
...
func TestNenhumComandoDaFichaPerdeAAba(t *testing.T) {
		...
		for _, achado := range postados.FindAllStringSubmatch(tela, -1) {
			...
			if !strings.HasSuffix(achado[1], "?tab="+aba.Valor) {
				t.Errorf("na aba %q o comando %q não carrega a aba: o clique joga o jogador para %q",
					aba.Valor, achado[1], asAbasDaFicha()[0].Valor)
			}
		}
	}
	...
	if vistos < 12*len(asAbasDaFicha()) {
		t.Fatalf("a varredura achou %d comandos nas sete abas, e são pelo menos %d: "+
			"a expressão parou de casar com o HTML", vistos, 12*len(asAbasDaFicha()))
	}
}
