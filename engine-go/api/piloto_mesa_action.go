...
func (s *Server) handleTableInitiative(w http.ResponseWriter, r *http.Request) {
...
func (s *Server) registerInitiativeTable(r *http.Request, campaignID, sessionID, d20 int64) error {
	user := currentUser(r)
	if _, _, _, err := s.sessionForCaller(r.Context(), user, campaignID, sessionID); err != nil {
		return err
	}
	_, _, eu := s.tableRoster(r.Context(), user, campaignID)
	if eu == nil {
