...
func (s *Server) chosenTabOf(ctx context.Context, sessionID, userID int64) string {
...
func (s *Server) pullTab(ctx context.Context, sessionID, userID int64) (aba string, puxado bool, deOnde string) {
...
func showTableIsTab(st *Server, c commandCtx) (*tabuleiro.BoardState, error) {
	...
	for _, aberto := range st.boards.OpenBoards(c.R.Context(), c.SessionID) {
		if aberto.ID == alvo {
			st.abas.Pull(c.SessionID, c.User.ID, alvo)
			return nil, nil
		}
	}
...
func swapBoard(st *Server, c commandCtx) (*tabuleiro.BoardState, error) {
