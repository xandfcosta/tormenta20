...
func stancesFromCatalog() map[string]stanceOfBook {
	stancesOnce.Do(func() {
		...
		for _, a := range activationsOfBook() {
			...
			flag := flags[a.ID]
			if flag == "" {
				flag = flags[a.ID+degrauDaPostura(flags, a.ID)]
			}
			if flag == "" {
				continue
			}
...
func degrauDaPostura(flags map[string]string, base string) string {
	for i := 1; i <= 9; i++ {
...
// flagsOfClassPowers mapeia id do poder → flag que os modificadores dele ligam.
func flagsOfClassPowers() map[string]string {
...
func (s *Server) effectsPanelOf(dto sheet.CharacterDTO) effectsPanel {
	...
	oferecidos := engine.ComputeItemEffects(s.catalogs.ActiveItemsFor(ec)).Conditional
	return effectsPanelFor(dto, oferecidos, s.catalogs.ComputeEquippedFlags(ec.Items))
}
...
func conditionRowsOf(dto sheet.CharacterDTO) []conditionRow {
	...
	linhas := []conditionRow{}
	for _, id := range parseConditionBlob(dto.ActiveConditions) {
...
// parseConditionBlob lê a coluna, e um blob quebrado vira NENHUMA condição.
//
// Nenhuma e não erro: a ficha inteira não pode deixar de abrir porque uma linha
// do banco tem JSON torto.
func parseConditionBlob(bruto string) []string {
	var ids []string
	if err := json.Unmarshal([]byte(bruto), &ids); err != nil {
		return nil
	}
	return ids
}
