...
func (s *Server) handleRulerTable(w http.ResponseWriter, r *http.Request) {
...
// rulerStops lê as paradas dos sinais, com a MIRA no fim.
//
...
// rulerReading escreve a medida em QUADRADOS, em metros e na faixa do livro.
//
...
func metersLeg(m engine.Measurement) string {
	if m.Squares == 0 {
		return ""
	}
	return meters(m.Metres) + "m"
}

// emptyRulerHint é o que a barra diz com uma ponta só posta.
//
...
const emptyRulerHint = "Clique para acrescentar paradas · duplo clique fecha · botão direito apaga"

// handleTemplateTable desenha o gabarito e diz QUEM está dentro.
//
...
func (s *Server) handleTemplateTable(w http.ResponseWriter, r *http.Request) {
...
func (s *Server) whoMeasuresTheTable(w http.ResponseWriter, r *http.Request) (papel string, sessionID int64, tabuleiroID string, ok bool) {
	...
	return papel, sessionID, s.chosenTabOf(r.Context(), sessionID, user.ID), true
}
...
// takesTemplateWho cruza a área com as peças da cena.
//
...
// templateDirection é o passo unitário do segundo clique, com ZONA MORTA.
//
...
func templateDirection(origem, mira engine.Square) engine.Square {
	...
	if moduloDe(dx) > 2*moduloDe(dy) {
		return engine.Square{X: direction(dx)}
	}
	if moduloDe(dy) > 2*moduloDe(dx) {
		return engine.Square{Y: direction(dy)}
	}
	return engine.Square{X: orDirection(dx), Y: orDirection(dy)}
}
...
// pointsTemplate diz quais formas precisam de um segundo clique. A esfera vai
// para todos os lados e o quadrado não aponta para lugar nenhum (p225).
func pointsTemplate(k engine.AreaKind) bool {
...
// templateSize trava a caixa de digitação em vez de recusar.
//
