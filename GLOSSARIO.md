...
| **ferramenta** (do mapa) | `$ferramenta`, `Ferramenta` | ~~pincel~~ (como sinal), ~~modo~~ | O que o clique no tabuleiro FAZ: mover, pintar uma das quatro espécies de terreno, ou marcar. **É um sinal só e o valor É a ferramenta**, então a exclusão é por construção — não há como duas estarem ligadas (ALE-203 na SPA, ALE-264 no piloto). O sinal chamava-se `$pincel` enquanto terreno era a única, e o nome ficou errado no dia em que MARCAR entrou. **Colide com a seção "Ferramentas" do trilho do mestre** (encontros, improviso), que é outra coisa: aquela é onde se vai, esta é o que o clique faz. A BORRACHA não é ferramenta — é um MODO do terreno (`$apagando`), porque ela não escolhe o que fazer, inverte o que a escolhida faz. |
