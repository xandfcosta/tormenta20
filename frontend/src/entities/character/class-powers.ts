...
/** How many elective slots a class entry has earned, spent and has left. */
export type ElectiveSlotUsage = { total: number; used: number; remaining: number }

/**
 * Slot accounting for ONE class entry. `used` counts only picks that could
 * have come from this class's pool (its own electives + any general power) —
 * a power taken from another class's list must not eat this class's slots in
 * a multiclass sheet.
 *
 * @example electiveSlotUsage('Bárbaro', 6, chosenIds) // { total: 5, used: 2, remaining: 3 }
 */
export function electiveSlotUsage(
  className: string,
  level: number,
  chosenIds: readonly string[],
): ElectiveSlotUsage {
  const { classPowers, generalPowers } = classPowerCandidates(className)
  const spendable = new Set([...classPowers, ...generalPowers].map((p) => p.id))
  const total = classSlotCount(className, level)
  const used = chosenIds.filter((id) => spendable.has(id)).length
