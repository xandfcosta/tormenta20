      ...
      <div class="flex min-h-0 flex-1 flex-col gap-3 px-4 py-3 lg:flex-row lg:gap-4">
        ...
        >
          {TRILHA.map((item) => (
            <a
              href={`#${item.id}`}
              class="shrink-0 rounded-none border border-grimorio-iron px-3 py-1.5 font-heading text-[11px] uppercase tracking-[0.16em] text-muted-foreground transition-colors hover:border-grimorio-gold hover:text-grimorio-gold"
            >
              {item.rotulo}
            </a>
          ))}
