...
/**
 * Read-only render of a server-computed sheet (the `ComputedSheet` payload
 * from `GET /characters/:id/sheet`). Shared by the standalone sheet page and
 * the in-session player view so both show identical stats. Pass `warnings`
 * to surface orchestrator avisos (the sheet page does; the match view hides
 * them to keep the play surface clean).
 */
export function ComputedSheetCards({
  computed,
  showWarnings = false,
}: {
  ...
  return (
    <Card>
      ...
      <CardContent className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
        <Stat
          label="PV"
          value={`${computed.vitals.pvCurrent}/${computed.vitals.pvMax}`}
        />
        <Stat
          label="PM"
          value={`${computed.vitals.pmCurrent}/${computed.vitals.pmMax}`}
        />
      ...
      <CardContent>
        <div className="grid grid-cols-2 gap-1 text-sm sm:grid-cols-3">
          {entries.map(([id, s]) => (
            ...
            >
              <span
                className={s.trained ? 'font-medium' : 'text-muted-foreground'}
              >
