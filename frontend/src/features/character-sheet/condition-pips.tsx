...
export function ConditionPips(props: { character: Character; class?: string; mini?: boolean }) {
  ...
  const chipClass = () =>
    props.mini
      ? 'max-w-20 truncate rounded-md border border-[color:var(--hp-hurt)]/60 bg-[color:var(--hp-hurt)]/15 px-1 text-4xs font-semibold uppercase tracking-wide text-[color:var(--hp-hurt)]'
      : 'rounded-md border border-[color:var(--hp-hurt)]/60 bg-[color:var(--hp-hurt)]/15 px-1.5 py-px text-3xs font-semibold uppercase tracking-wide text-[color:var(--hp-hurt)]'
