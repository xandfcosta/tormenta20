  ...
  return (
    <Show when={props.item} fallback={<EmptySlot label={props.label} wide={props.wide} />}>
      {(item) => (
        <div
          class={cn(
            'relative min-h-[3.75rem] rounded-sm border border-grimorio-gold/40 bg-[var(--grimorio-panel-raised)] px-2.5 py-2 text-left',
...
function EmptySlot(props: { label: string; wide?: boolean }) {
  return (
    <div
      class={cn(
        'flex min-h-[3.75rem] flex-col justify-center rounded-sm border border-dashed border-grimorio-iron bg-[var(--grimorio-panel)] px-2.5 py-2',
        props.wide && 'col-span-2',
      )}
    >
