...
| **poço** | `Pools`, `pvMax`/`pmMax` | ~~vitais~~ (para o par máximo+atual), ~~stats~~ | O par PV e PM de uma ficha: o MÁXIMO, que é **derivado** do catálogo, e o ATUAL, que é `máximo − dano`. Nenhum dos dois é estado: se o catálogo mudou, as regras do mundo mudaram, e os personagens acompanham (decisão do dono, ALE-355). Quem deriva e grava é o **funil** (`sheet.ApplyToPools`), e é o único caminho — ver `TestEveryVitalWriteGoesThroughTheFunnel`. As colunas `hpMax`/`hpCurrent`/`mpMax`/`mpCurrent` de `characters` sobrevivem como ESPELHO para quem lê a linha por JOIN, e saem por migração quando o último leitor sair. |
