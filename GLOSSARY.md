...
| **emenda** | `engine.Amendments`, `campaign_items`, `campaign_grants`, `campaign_silences` | ~~remendo~~, ~~regra da mesa~~, ~~patch~~ | O que a mesa MUDA no **mundo** dela, em três espécies. **Emenda de verbete**: a campanha ACRESCENTA modificador a uma entrada do livro — na mesa do dono, o medalhão de prata concede +1 em Luta além do +1 de limite de PM da p160. Ela muda o que uma COISA é, e por isso vale para o mundo inteiro. **Concessão** (`CampaignGrant`): a mesa dá modificadores a quem o **seletor** alcança; muda quem TEM as coisas, e por isso carrega escopo. **Silêncio** (`Silence`): um **termo** que esta mesa não aplica. Nenhuma delas substitui o livro — a de verbete acrescenta, e o silêncio remove um termo nomeado. **Não é a regra opcional**, que é o punhado de interruptores que o próprio livro oferece. `remendo` está proibido porque já é o quadro que o Datastar aplica na tela. **Quem autora emenda é o MESTRE** — jogador não muda regra, ele só liga o opt-in que a ficha dele já oferece, e quem cobra isso é o servidor (ALE-387). |
| **seletor** | `engine.Selector` | ~~alvo~~, ~~escopo~~ (sozinho) | A quem uma **emenda** alcança: a mesa inteira (`EveryoneIn`) ou uma ficha (`OnlyCharacter`). No banco é a coluna `characterId`, e NULA quer dizer a mesa inteira. O id é o do CLONE que vive na campanha — o **molde** do elenco não está em mesa nenhuma. É o encaixe onde entra "todo anão ignora a arma desbalanceada" sem mudar o desenho. |
| **termo** | `engine.TermID` | ~~modificador~~ (quando se fala de endereço), ~~linha~~ | UMA contribuição dentro de uma fonte — o `+1 de Luta` do medalhão, distinto do `+1 de limite de PM` do mesmo medalhão. O endereço dele é `fonte::alvo::escala::condição`, e **não carrega o valor nem a prosa**: assim uma errata do livro não evapora o **silêncio** que o mestre escreveu. A escala e a condição são o que separa os pares que existem (os dois `maxPv` do anão, os dois `pmCost` do druida). Quem garante que o endereço endereça um termo só é o `TestEveryCatalogTermHasAUniqueAddress`. O MESMO endereço serve ao opt-in do jogador (`character_conditionals`) e ao silêncio do mestre (`campaign_silences`): eram dois, e convergiram na ALE-387. Quando os condicionais dividem uma flag, quem endereça o conjunto é o `FlagGroupID` (`flag:<nome>`) — e as duas famílias de chave não colidem porque endereço de termo sempre tem `::`. |
...
| **gravação falhando** | `SaveFailed` | ~~sujo~~, ~~dirty~~ (na tela) | A última tentativa de escrever o TABULEIRO no disco não deu certo, e ele continua rodando de MEMÓRIA. É estado e não notícia: vale enquanto durar, então a tela o LÊ do store a cada quadro em vez de esperar um aviso que pode se perder (ALE-288). **A FILA saiu desta pergunta na ALE-371**: a gravação dela mora dentro da mutação, e o comando que o disco recusa é recusado na hora, com a frase — não há estado sujo para mostrar. O `saveFailed` do `table/routes.go` pergunta só ao tabuleiro, e some quando ele fizer a mesma travessia. **Só o mestre vê** — é ele quem pode parar a sessão e chamar alguém; para o jogador seria um aviso sobre o qual ele não pode fazer nada. |
...
| **uid** | `uid` | ~~guid~~, ~~key~~, ~~identifier~~ | O identificador OPACO e ESTÁVEL de um verbete do catálogo (`pwr_k3m9x2ft4q`). É o que o banco guarda e o que um catálogo usa para apontar para outro; ele nunca muda. Não confundir com o **id**, que é o nome em kebab-case, é público (a URL o mostra) e muda quando o nome muda. Um verbete tem os dois (ALE-402). |
...
| **aprimoramento exclusivo** | `exclusive` | ~~sozinho~~, ~~único~~ | O aprimoramento que NÃO pode ser escolhido junto com nenhum outro na mesma conjuração. O livro diz isso em três lugares: como regra geral do **truque** (*"truques não podem ser usados em conjunto com outros aprimoramentos"*, p171) e, escrito à mão, em dois aprimoramentos comuns — a esfera de invisibilidade da Invisibilidade (p195) e a Luz permanente de pó de rubi (p197). Por isso a marca é do APRIMORAMENTO e não do truque: modelar "é truque" deixaria os outros dois de fora. Quem recusa é o servidor (`validateAugments`). |
| **truque** | `Truque:` (prefixo da descrição) | ~~cantrip~~, ~~magia menor~~ | O aprimoramento que transforma a magia em uma versão mais simples e **zera o custo em PM** (p171). São **catorze** no livro, um por magia, e todos são **exclusivos** pela regra geral. **Não é um círculo de magia** — a magia continua no círculo dela —, mesmo o app numerando o truque como círculo 0 no custo e na tela (ver **círculo**). Na tela ele aparece pelo prefixo `Truque:` na descrição, que é como o livro o imprime. |
| **tipo de efeito** | `effect-types` | ~~tag~~, ~~categoria~~ | A família de um efeito — **Medo**, **Mental**, **Movimento**, **Metabolismo** (T20 p228). O livro a imprime em itálico no fim da condição ("Abalado … *Medo*."), e ela é o que decide imunidade: "criatura com imunidade a medo não será afetada por efeitos do tipo medo". No dado a condição a guarda como CHAVE (`tags: ["cansaco"]`, sem acento); na tela sai o nome do livro. Ganhou catálogo próprio na ALE-264 porque não havia para onde o elo apontar. |
| **elo** | `eloParaOAcervo` | ~~link~~ (no código) | A palavra que leva ao verbete dela: a tag da condição leva ao tipo de efeito, "Agrava para Apavorado" leva à condição, o poder concedido leva ao poder. **Não é o botão do livro** — o elo anda DENTRO do acervo, o botão sai para o PDF. Os dois usam o mesmo sublinhado pontilhado de propósito: são o mesmo gesto ("leva para outro lugar") e desenhá-los diferente ensinaria que não são. O clique MOSTRA o verbete numa caixa sobre a cena; o `href` (`/mestre/<catálogo>?entrada=<id>`) fica para o ctrl+clique e o link copiado. |
...
| **combatente** | `InitiativeEntry` | ~~linha~~ (em texto visível) | Uma entrada na fila. "Linha" só vale falando do elemento de UI, nunca do objeto. **Com personagem atrás, o PV e o PM dela são ESPELHO da ficha e nunca a autoridade**: os gestos da fila escrevem na ficha e redesenham a linha, e o `RefreshCharacterVitals` repergunta o poço a cada desenho da Mesa — que é o que faz um jogador que bebe uma poção mover a fila do mestre em até um segundo (ALE-358). **Sem personagem atrás — o NPC digitado — a entrada É o registro**, e nada a sobrescreve: a fronteira é o `CharacterID` nulo, e é a mesma que o `DeltaVitals` e o `PatchVitals` usam. |
| **faixa** (de quem vem depois) | `turnAhead`, `turnStripOf`, `UpcomingTurns` | ~~ordem~~, ~~tira~~, ~~próximos turnos~~ | A linha do cabeçalho da Mesa que diz a **vez** de agora e as duas seguintes, na ordem da **fila**, DANDO A VOLTA (ALE-179, na tela desde a ALE-290). A volta é a razão de ela existir: no último da **rodada**, quem vem depois está no TOPO da lista, e quem lê de cima para baixo não acha ninguém. O `⟲` marca onde a rodada seguinte começa. **São TRÊS**, e o número é escolhido e não medido — o servidor não sabe quanto cabe na caixa. Quem quer a ordem inteira tem o trilho e a gaveta. **Não é o botão de avanço**, que diz *"Próximo: Fulano"* e continua dizendo: a faixa é onde se LÊ a ordem, o botão é o que o CLIQUE faz. A repetição do nome é deliberada. Ela escreve **você** no lugar do nome quando UM dos três é de quem olha; com dois, o nome volta — a palavra deixa de desambiguar e passa a atrapalhar. Vazia fora de combate, e aí o cabeçalho volta a dizer "Aguardando iniciativa". |
...
| **cena** | `live.Scene`, `SceneKind` | ~~modo~~, ~~fase~~ | O que o mestre liga e desliga; enquanto ela dura, a mesa recebe a fila (ALE-210). Ela tem ESPÉCIE desde a ALE-365 — **ação**, **exploração** e **interpretação** (p252) —, e só a de ação conta **rodada** e cobra o **turno**: nas outras duas o tempo corre solto e gesto nenhum custa ação. Começar uma INTERROMPE a que estiver em jogo, que é o exemplo do livro (a conversa vira combate). **Ver a colisão C1.** |
| **rodada / vez** | `round` / `turnIndex` | ~~turno~~ para "de quem é a vez" | "Turno" é o que se GASTA; "vez" é de quem é. |
| **turno** | `TurnBudget`, `StandardLeft` / `MovementLeft` | ~~ações~~, ~~pontos de ação~~ | O que cada um tem para gastar na **vez** dele: UMA **ação padrão** e UMA **ação de movimento** (p233). São dois booleanos e não um contador de dois, porque a troca entre eles é de MÃO ÚNICA. O rodapé da Mesa escreve o que sobrou — *padrão e movimento*, *padrão (dá para mover)*, *movimento*, *sem ação* — e ele se renova inteiro para quem entra na vez. |
| **ação padrão / ação de movimento / ação completa** | `ActionStandard`, `ActionMovement`, `ActionFull` | ~~ação principal~~ (no código), ~~turno~~ | Os custos do livro (p233). A **padrão** ataca e conjura; a **de movimento** anda; a **completa** exige as DUAS e não "o que sobrar". A troca é de mão única: a padrão vira uma de movimento, e nunca o contrário. `livre`, `reação` e `passivo` não gastam nada. A tela ainda diz *ação principal* na legenda do movimento — é prosa antiga, e o nome do livro é **ação padrão**. |
| **ataque** | `engine.ResolveAttack`, `live.PendingAttack`, `combat.Strike` | ~~golpe~~ (na tela) | O teste de Luta ou Pontaria contra a **Defesa** do alvo (p230): acerta com total IGUAL ou maior, e o 20 natural sempre acerta e o 1 sempre erra (p221). O **crítico** exige acertar E rolar na margem, e multiplica só os DADOS — *"1d8+3 torna-se 2d8+3"* (p231, p142); a **RD** subtrai do dano (p229). Custa a **ação padrão** (*"Agredir"*, p233), e quem não pode agir não ataca. O jogador PROPÕE — o d20 rola e a conta inteira aparece na faixa como rascunho — e só o mestre CONFIRMA, que é quando o dano pousa e a padrão é cobrada: a mesma divisa do **movimento**. O provisório mora na **fila** e não no tabuleiro, porque combate sem mapa aberto continua sendo combate. Nesta fatia só **personagem** ataca: o verbete do bestiário traz o ataque como texto, e texto não resolve. |
| **duração** | `engine.Duration`, `active_effects.scope` | ~~escopo~~ (na tela), ~~`defaultScope`~~ | Por quanto tempo uma habilidade mantém o efeito dela — uma das **seis** da p227: instantânea, cena, **sustentada**, definida, permanente e descarregar. **Quem manda é a MAGIA**: o efeito guardado na ficha recebe a duração dela, e não uma segunda transcrição — o catálogo tinha as duas, em duas línguas, e oito divergiam (ALE-365). O `defaultScope` sobrevive só onde a magia não pode responder: a **instantânea**, cuja consequência não é a magia (*"Curar Ferimentos age instantaneamente, mas os ferimentos continuam curados"*), e a **definida sem quantia**, que é espécie e não medida. **A definida COM quantia manda como qualquer outra**, e a medida mora em prosa no `durationNote` (*"pode ser medida em rodadas, horas, dias ou outra unidade de tempo"*): o app lê as que sabe EXPIRAR — hoje `1 turno` e `1 dia` —, e nota que ele não conhece continua sendo definida sem quantia. Medir "3 rodadas" sem ninguém contar rodadas gravaria um efeito ETERNO com cara de medido. `escopo` está proibido na tela porque não é palavra do livro; a coluna continua `scope` e não se conserta por palpite — renomeá-la mexe em toda ficha gravada. |
| **reação** | `ActionReaction`, `UsableNow`, `ActionMoment` | ~~resposta~~, ~~interrupção~~ | A habilidade que *"acontece em resposta a outra coisa"* e é *"uma resposta automática, que pode ocorrer mesmo fora do seu turno"* (p233). Ela não custa nada do **turno** — nisso é igual à **ação livre** —, e o que a separa da livre é o INSTANTE: a livre é *"uma escolha consciente, feita no seu turno"*, e a reação vale fora da vez e **mesmo de quem não pode agir**, *"como por estar atordoado"*. São dois eixos e não um: o `TurnBudget` responde se o custo CABE, o `UsableNow` responde se é a HORA, e com um só não há como distinguir livre de reação. |
| **duração de uma vez** | `UnitTurn`, `engine.TurnScope()`, `scope: "turn"` | ~~1 rodada~~, ~~turno~~ (na coluna) | A duração que o livro imprime como **"1 turno"** — uma só magia a tem, o Escudo da Fé (p192), e ela é conjurada por **reação**. Ela acaba com a **vez EM CURSO**, que pode não ser a de quem carrega o efeito: uma reação acontece fora da vez de quem reage, e ancorá-la em quem recebe daria a ele uma **rodada** inteira de escudo. Por isso a expiração varre a FILA toda no giro da vez, e o fim da **cena** a leva também — a reação que nunca chegou a girar precisa de outra ponta. A aba Efeitos escreve *"até o fim da vez"*, com a palavra do glossário; a decomposição escreve *"1 turno"*, com a palavra do livro. |
| **morrendo / estável / morto** | `VitalDying`, `VitalStable`, `VitalDead`, `engine.VitalStateOf` | ~~caído~~ (é outra condição, p394), ~~desmaiado~~ | Os três estados abaixo de 1 PV (p236). **Morrendo** é estar a 0 PV ou menos COM a condição Sangrando: no início da vez, teste de Constituição (CD 15); passar estabiliza, falhar tira 1d6 PV. **Estável** é estar a 0 ou menos SEM sangrar — não é uma terceira condição, e por isso não há coluna: volta a sangrar se perder mais PV. **Morto** é chegar a –10 ou a menos a metade dos PV totais, o que for mais baixo (o Oberon de 12 PV morre em –10; o de 30, em –15). Nos três o personagem está **inconsciente**: não age e **não reage** — "sem ações (incluindo reações)", p395; a isenção da reação na p233 é do atordoado (decisão do dono, ALE-366). Voltar a 1 PV ou mais acorda. `~~caído~~` está proibido para este sentido porque Caído é a condição de estar no chão, com outra regra. |
| **manutenção** | `TurnUpkeep`, `PaySustained` | ~~upkeep~~ (na tela), ~~custo de turno~~ | O 1 PM que cada habilidade **sustentada** cobra de quem ENTRA na vez (p227), e a linha da faixa que diz o que foi pago e o que caiu: *"Velocidade e Oração · −2 PM (58 → 56)"*. É **ação livre**, então não gasta o **turno** — quem sustenta entra na vez com a padrão e a de movimento inteiras. Sem mana, *"a habilidade termina"*: o efeito sai da ficha e a faixa nomeia qual. **Quem está a 0 PV também não sustenta** — manter é ação livre e a 0 PV *"você cai inconsciente"* (p236) —, e é por aí que a cláusula da morte da p227 chega a um app cujo poço de PV tem piso em zero. A faixa diz as duas razões com frases diferentes, porque curar e reconjurar são decisões diferentes. **Não é o custo de conjurar**, que é pago uma vez, na execução. A ordem de pagamento é da mais antiga para a mais nova, e ela só importa quando o mana não cobre todas — o livro deixa a escolha com o jogador, e quem quiser outra encerra a que preferir, o que também é ação livre. |
| **recuperação** | `rest` | — | O painel. Dentro dele mora UMA ação de descanso — o **descanso de dia**, que é a palavra do livro (T20 p106, "Recuperando PV e PM") e devolve PV e PM. Aqui esta linha dizia que eram DUAS, *"descanso de cena e descanso de dia"*, e a de cena nunca devolveu ponto nenhum: ela expira a **duração de cena**, e o glossário mentia junto com o botão (ALE-233). Ver **expirar efeitos de cena**. |
...
| **poço** | `Pools`, `pvMax`/`pmMax` | ~~vitais~~ (para o par máximo+atual), ~~stats~~ | O par PV e PM de uma ficha: o MÁXIMO, que é **derivado** do catálogo, e o ATUAL, que é `máximo − dano`. Nenhum dos dois é estado: se o catálogo mudou, as regras do mundo mudaram, e os personagens acompanham (decisão do dono, ALE-355). Quem deriva e grava é o **funil** (`sheet.ApplyToPools`), e é o único caminho — ver `TestEveryVitalWriteGoesThroughTheFunnel`. Quem precisa do poço de uma MESA inteira usa o `sheet.PoolsForCharacters`, que faz o grupo em seis consultas. As colunas `hpMax`/`hpCurrent`/`mpMax`/`mpCurrent` de `characters` **não existem**: saíram na migração 00015, e com elas os gestos que só existiam para mantê-las. |
| **dano** | `hpDamage`/`mpSpent`, `character_damage` | ~~PV atual~~ como estado, ~~vida~~ | O que o banco de fato GUARDA: quanto o personagem apanhou e quanto de PM gastou. A ausência de linha quer dizer INTACTO — só quem apanhou tem registro (migração 00014), e voltar ao cheio APAGA a linha em vez de gravar um par de zeros. É a inversão que faz subir de nível entregar os PV novos preenchidos e um máximo que encolhe levar o atual junto, sem ninguém calcular delta nenhum. **Não confundir com o dano de uma arma** (`DamageAll`), que é a rolagem — o contexto separa. |
| **PV temporário** | `tempHp` | ~~escudo~~, ~~colchão~~, ~~PV extra~~ | A reserva que o dano gasta ANTES do PV — *"pontos temporários são sempre os primeiros a serem gastos"* (p106). Ela **SOMA** entre fontes: duas magias de melhoria convivem, e a p106 diz *"são somados a seus pontos atuais, mesmo que ultrapassem o máximo"*. Aqui morava um vale-o-maior citando uma p256 que fala de sementes de aventura — a frase não existe no livro (ALE-347). **Na tela é uma PARCELA e nunca um somando**: `137/137 +70`, e não `207/137`, porque a barra responde *"quanto apanhei"* e somar faria um herói ferido desenhar cheio. A cor é o `--hp-temp`, azul PRÓPRIO: o único azul da casa era o do mana, e na barra de VIDA ele diria PM. |
...
| **rascunho de lugar** | `EditPlace`, `draftView`, `placeDraftBase` | ~~cortina~~, ~~preparação~~, ~~modo de edição~~ | A cena de um **lugar** sendo montada FORA da sessão, em `/campanhas/{id}/lugares/{id}` (ALE-292). É a MESMA superfície do **tabuleiro** apontada para o acervo em vez de para a mesa: mesmo trilho, mesmos pincéis, mesma **peça** avulsa, mesmos **marcadores**, mesma **régua** e mesmo **gabarito** (ALE-293 — *"cabe a bola de fogo nesta sala?"* é pergunta de preparação). O gabarito conta a peça ESCONDIDA aqui, ao contrário da Mesa: não há outro papel para redigir, e é justamente a emboscada que o mestre está medindo. O que ela não tem é tudo o que precisa de uma mesa do outro lado — **cortina**, lente, **aba**, acervo, encerrar, **pôr no mapa** — e o **arrasto** significa outra coisa: na mesa ele PROPÕE um **movimento** com custo e vez, aqui ele põe a peça onde foi solta e acabou. **Não é a cortina**, e a linha entre as duas é o TEMPO: a cortina é durante a sessão, com a mesa presente; o rascunho é na quinta-feira, sem ninguém conectado. Decisão do dono: os dois convivem. **Não se monta o lugar que está aberto numa mesa** — encerrar a aba chama o `Archive`, que sobrescreve o lugar de mesmo nome, e o trabalho do rascunho sumiria em silêncio. Ele grava a CADA GESTO, e a tela diz isso: não há botão de salvar. |
...
| **cromo** | `chrome`, `TopChromeRows` | ~~moldura~~, ~~barra~~, ~~overlay~~ | O que flutua SOBRE o mapa e não é mapa: o painel do nome com as **abas**, o painel dos verbos, o trilho de **ferramentas**, os avisos e os ajudantes. Ele nasceu na ALE-203, quando a barra de cima deixou de ser uma linha acima da cena — flutuando, o tabuleiro cresce até o topo. **Cobrir a vista e roubar o clique são promessas diferentes**, e a segunda nunca foi feita (ALE-294): cada painel devolve ao mapa o fundo entre os próprios controles, e quem prende isso é o `TestNoChromeOverTheMapStealsTheClickOffItsControls`. O cromo do TOPO tem consequência no servidor: a **janela** nasce com o quadrado (0,0) na quina de cima, então as duas primeiras fileiras do plano ficam debaixo dele no zoom padrão, e **pôr no mapa** faz a **peça** nascer abaixo delas. `~~moldura~~` está proibida pela razão da linha acima. **Colide com o navegador Chrome**, que aparece em quatro comentários deste repositório (o leitor do livro): o identificador é sempre composto (`TopChromeRows`), nunca `chrome` sozinho. |
...
`ownerId` do personagem. Quem lista os membros não filtra papel NENHUM — quem
responde "esta pessoa mestra?" é o `roleIn`, comparando com o dono da campanha,
e é a mesma verdade que autoriza.
...
| **a cena da sessão** | `table` | `web/table` (ALE-278) — a cena de `/campanhas/{campanha}/sessoes/{sessao}`: o palco, a fila, o elenco, as notas, a cortina e o tabuleiro inteiro. **O ENDEREÇO dela deixou de dizer "mesa" na ALE-345**, e o motivo está na primeira seção deste arquivo: `/mesa/{c}/{s}` usava a palavra para a SESSÃO, que é exatamente o que a linha de **mesa** proíbe — a regra existia e a URL a violava. O pacote continua `table` por enquanto, e renomeá-lo é decisão separada. Ela NÃO precisa de sufixo como o `sheetui`, porque não há uma camada `table` de dado para colidir: o estado ao vivo é `aovivo` e o mapa é `tabuleiro`. **Cuidado com a outra acepção**: `table` de banco de dados aparece só no `cmd/seed`, que é outro pacote e não é domínio |
...
| **fronteira** (do pacote) | `boundary` | os 25 `boundary_test.go`, um por pacote (ALE-301). É a porta que uma cena declara para o hospedeiro — **não confundir com `port`**, que é o TIPO da porta; `boundary` é o guarda que prova que ninguém a contorna |
